...
# Minimax algorithm with Alpha-Beta Pruning and debug info
def minimax(alpha, beta, is_maximizing):
    print("Current board:")
    print_board()
    print(f"Alpha: {alpha}, Beta: {beta}, Is Maximizing: {is_maximizing}\n")
    ...
    if is_maximizing:
        ...
        for i in range(9):
            if board[i] == "":
                ...
                print(f"Maximizing - Updated Best Score: {best_score}, Alpha: {alpha}")
                if beta <= alpha:
                    print("Pruning branch for maximizer\n")
                    break  # Pruning
        return best_score
    else:
        best_score = float('inf')
        for i in range(9):
            if board[i] == "":
                ...
                print(f"Minimizing - Updated Best Score: {best_score}, Beta: {beta}")
                if beta <= alpha:
                    print("Pruning branch for minimizer\n")
                    break  # Pruning
        return best_score
    ...
    for i in range(9):
        if board[i] == "":
            ...
            if score > best_score:
                best_score = score
                best_move = i
            print(f"Evaluating move {i}: Best Score so far: {best_score}\n")
...
for i in range(9):
    button = tk.Button(window, text="", width=10, height=3, font=("Arial", 30),
